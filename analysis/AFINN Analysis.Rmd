...
# Load datasets
approvalurl<- getURL("https://raw.githubusercontent.com/bwaite15104/bwaite15104.github.io/master/data/Presidential%20Approval%20Ratings.csv")

approval<-tbl_df(read.csv(text = approvalurl,header=TRUE, sep=",", stringsAsFactors = FALSE))


presidentsurl<-getURL("https://raw.githubusercontent.com/bwaite15104/bwaite15104.github.io/master/data/president_terms.csv")

presidents<-tbl_df(read.csv(text = presidentsurl,header=TRUE, sep=",", stringsAsFactors = FALSE))

#spesenturl<-getURL("https://raw.githubusercontent.com/bwaite15104/bwaite15104.github.io/master/data/speech_polarity_for_model_AFINN.csv")

#speech_sentiments <- (read.csv(text=spesenturl, header=TRUE, sep=",",stringsAsFactors = FALSE))

speech_sentiments<-tbl_df(read.csv("speech_polarity_for_model_AFINN.csv", header=TRUE, stringsAsFactors=FALSE))
...
#speech_sentiments2$speech_date<-as.Date(speech_sentiments$speech_date)
...
monthly_sentiment <- speech_sentiments %>%
  ...
  select(president, speech_date, speech_year, sentiment, speech_diversity) %>%
  group_by(president,
           speech_year,
...
boxplot(sentiment_vs_approval$avg_sentiment) # A lot of outliers
...
sentiment_vs_approval %>%
  ...
  labs(x = 'Monthly Average Approval Rating', 
       ...
       title = 'AFINN Sentiment vs Approval Rating')

# Taking a look at a linear regressio model output fit to all of the data
all_speeches_approval_lm <- lm(avg_approval ~ avg_diversity + avg_sentiment, data = sentiment_vs_approval)
summary(all_speeches_approval_lm) # very weak rSquared at about 13%, adjusted R2 of .02

all_speeches_approval_lm2<- lm (avg_approval ~ log(avg_sentiment), data= sentiment_vs_approval)
...
for (i in seq(0.3, 1, by = 0.01)) {
  ...
  # Fit model
  all_speeches_approval_lm <- lm(avg_approval ~ avg_diversity + avg_sentiment, data = filtered_lm_df)
...
all_speeches_approval_lm1 <- lm(avg_approval ~ avg_diversity + avg_sentiment, data = filtered_lm_df)
...
# grow tree 
tree1 <- rpart(avg_approval ~ avg_diversity + avg_sentiment, method="anova", data=sentiment_vs_approval)
...
set.seed(17)
fit <- randomForest(avg_approval ~ avg_diversity + avg_sentiment, data=sentiment_vs_approval)
